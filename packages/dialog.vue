<template>
  <!-- 对话框的遮罩层 self只有点击自己才触发-->
  <transition name="dialog-fade">
    <div class="k-dialog__wrapper" v-show="visible" @click.self="handleclose">
      <div class="k-dialog" :style="{ width: width, marginTop: top }">
        <div class="k-dialog__header">
          <slot name="title"
            ><span class="k-dialog__title">{{ title }}</span></slot
          >
          <button class="k-dialog__headerbtn">
            <i class="k-icon-close" @click="handleclose"></i>
          </button>
        </div>
